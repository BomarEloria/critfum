...
Weapon Fumble App
Version: v1.0.3
Styled UI + FULL VERBATIM TABLES (LOCKED)
*/
...
const FUMBLE_CAUSES = [
  ...
  "cut your own belt and your pants trip you"  
];
...
export default function WeaponFumbleApp() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1e293b,#0f172a)", display:"flex", justifyContent:"center", alignItems:"center" }}>
      <div style={{ background:"white", padding:20, borderRadius:12, maxWidth:520, width:"100%", boxShadow:"0 10px 30px rgba(0,0,0,0.3)" }}>
        ...
        <input type="number" min={1} max={100} value={roll} onChange={e=>setRoll(Number(e.target.value))} style={{ width:"100%", marginBottom:10, padding:8,border: "2px solid #1e40af" }} />

        <button onClick={generateFumble} style={{ width:"100%", padding:10, background:"#2563eb", color:"white", border:"2px solid #1e40af", borderRadius:6 }}>🎲 Resolve Fumble</button>

        {result && (
          <div style={{ marginTop:20, padding:12, background:"#f1f5f9", borderRadius:8 }}>
            <p><b>Cause:</b> {result.cause}</p>
            <p><b>Result:</b> {result.effect}</p>
          </div>
        )}
      </div>
    </div>
